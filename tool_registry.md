...
- Main behavior: draws an n8n-style workflow story map first when a pipeline is detected, then keeps grouped layers for app parts, files, tools, pipelines, and risk zones. Clicking a station or node aims SMAC; `Pull trigger` builds a scoped `$smac` sniper prompt.
...
- Map display behavior: workflow view shows large processing stations with important arrows only; overview keeps high-level wires; files, tools, and pipelines are grouped into readable cards instead of listing every detected file or dependency as a node.
